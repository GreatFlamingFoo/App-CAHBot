...
my $nickname = 'CAHBot' . $$;
my $ircname	 = 'Cards Against Humanity Bot by greatflamingfoo';
...
sub run
{
	...
	my $dbh = DBI->connect("dbi:SQLite:dbname=CAHBot.sqlite", "", "") 
		or die "Oh noooo! ".$DBI::errstr;

	POE::Session->create(
		package_states => [
			'App::CAHBot' => [ qw(_default _start irc_001 irc_public irc_msg
				irc_join deal list prompt vote results) ],
		],
		inline_states => {
			irc_part => \&part,
			irc_quit => \&part,
...
sub prompt
{
	...
	$kernel->delay( "prompt", 3600 );
	$kernel->delay( "vote", 600 );
	unless(@bcards)
	{
		@bcards = shuffle(@bdiscard);
		@bdiscard = ();
	}
	push @bdiscard, (my $card = pop @bcards);
	$kernel->yield("privmsg", [@channels], $card);
	$heap->{'state'} = "playing";
}
...
{
	my($kernel, $heap) = @_[KERNEL, HEAP];
	$kernel->delay( "results", 600 );
	# privmsg [@channel], white cards
	$heap->{'state'} = "voting";
}

sub results
{
	# privmsg [@channel], winner
	# foreach user deal and list
}
...
{
	my($user) = $_[ARG0];
	# privmsg user, card foreach @hand
}
...
#	$irc->pluggin_add('BotCommand', POE::Component::IRC::Plugin::BotCommand->new());
...
sub irc_001 {
	...
	$irc->yield(names => @channels);
	
	# for each user deal x 10 and list
	$irc->yield("prompt");
	return;
}
...
{
	my ($sender, $who, $where, $what) = @_[SENDER, ARG0 .. ARG2];
	$sender->get_heap()->shutdown() if $what =~ /quit/i;
}
